...
public class MensagemService {
	...
	@Autowired
	private MensagemCustomRepository mensagemCustomRepository;
	
	@Autowired
	private AgendamentoCustomRepository agendamentoCustomRepository;
	
	@Autowired
	ClienteCarteiro clienteCarteiro;
	...
	public MensagemConvidadoDTO buscaQuantidadeMensagemPorConvidado(Integer idConvidado, Integer idAgendamento) {
		...
		mensagemConvidadoDTO.setQtMsgConfirmacao(repository.buscaQuantidadeMensagemConfirmacaoPorConvidado(agendamentoConvidado.getAgendamento().getId(), agendamentoConvidado.getConvidado().getId()));
		mensagemConvidadoDTO.setQtMsgAlteracao(repository.buscaQuantidadeMensagemAlteracaoPorConvidado(agendamentoConvidado.getAgendamento().getId(), agendamentoConvidado.getConvidado().getId()));
		mensagemConvidadoDTO.setQtMsgCancelamento(repository.buscaQuantidadeMensagemCancelamentoPorConvidado(agendamentoConvidado.getAgendamento().getId(), agendamentoConvidado.getConvidado().getId()));
	...
	private MensagemConvidadoDTO preparaMensagemConvidado(ConvidadoDTO convidado, Integer idAgendamento) throws IOException{
		...
		if(convite != null) {
			mensagemConvidadoDTO = new MensagemConvidadoDTO();
			...
			mensagemConvidadoDTO.setAgendamentoConvidado(convite);
			mensagemConvidadoDTO.setDataAgendamento(convite.getAgendamento().getDataAgendamento());
			mensagemConvidadoDTO.setNomeConvidado(convite.getConvidado().getNomeConvidado());
			mensagemConvidadoDTO.setNomeDeputado(convite.getAgendamento().getNomePatrocinador());
			mensagemConvidadoDTO.setLocal(convite.getAgendamento().getLocal());
			mensagemConvidadoDTO.setPeriodo(PeriodoEnum.getEnumByName(convite.getAgendamento().getPeriodo().getId()));
			mensagemConvidadoDTO.setEmailConvidado(convite.getConvidado().getEmail());
			mensagemConvidadoDTO.setPathQrCode(FotoUtil.converterPathToBase64(file.toPath()));
		}
		return mensagemConvidadoDTO;
	}
		...
		for (ConvidadoDTO convidado: mensagemConvidado.getConvidados()) {
			...
			if (content != null) {
				...
				RequisicaoEnvioEmail requisicao = RequisicaoEnvioEmail.builder().solicitante(EmailUtil.getSolicitante())
						.mensagem(msg).build();
				try {
					clienteCarteiro.enviarEmail(requisicao);
	...
	public List<MensagemConvidadoDTO> buscaPorParametros(Map<String, String[]> filter) {
		...
		List<MensagemConvidado> listaMensagemConvidado = mensagemCustomRepository.buscaPorParametros(nome, dtInicio, dtFim, idTipoMensagem, idAgendamento);
		List<MensagemConvidadoDTO> retorno = new ArrayList<>();
		
		for(MensagemConvidado mensagemConvidado : listaMensagemConvidado) {
			MensagemConvidadoDTO mensagemConvidadoDTO = new MensagemConvidadoDTO();
			mensagemConvidadoDTO.setDataAgendamento(agendamento.getDataAgendamento());

			if(mensagemConvidado != null) {
				ConvidadoDTO convidadoDTO = convidadoService.buscaConvidadoMensagemConvidadoPorId(mensagemConvidado.getId());
				mensagemConvidadoDTO.setConvidado(convidadoDTO);
				...
				if(fotoConvidado != null) {
					mensagemConvidadoDTO.setFotoConvidado(fotoConvidado.getImagemFoto());
				}

				mensagemConvidadoDTO.setNomeConvidado(convidadoDTO.getNomeConvidado());
				mensagemConvidadoDTO.setDataEnvio(mensagemConvidado.getDataEnvio());
				mensagemConvidadoDTO.setTipoEmail(mensagemConvidado.getTipoMensagem().getTipoMensagem());
				retorno.add(mensagemConvidadoDTO);
			}
		}
