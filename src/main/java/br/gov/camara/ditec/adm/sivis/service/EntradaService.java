...
public class EntradaService {
	...
	@Autowired
	private RestricaoEntradaRepository restricaoRepository;
	...
	public void registroEntradaAgendamento(EntradaDTO entradaDTO, Visitante visitante, Integer idAgendamento) throws NegocioException {
		...
		// Validação comentada para permitir várias entradas de uma pessoa no mesmo agendamento.
		// [Alterado por : Kelton Ribeiro - 06/03/20]
	...
	public List<ResultadoPesquisaVisitanteEntradaDTO> pesquisarVisitantesEntradas(
		...
		TipoRestricaoDTO tipoRestricaoDTO = filtroPesquisarVisitanteEntradaDTO.getTipoRestricaoDTO() != null
				...
				: null;
		// Entradas
		LocalDate dataEntradaInicial = filtroPesquisarVisitanteEntradaDTO.getDataEntradaInicial() != null
				? filtroPesquisarVisitanteEntradaDTO.getDataEntradaInicial()
				: null;
		LocalDate dataEntradaFinal = filtroPesquisarVisitanteEntradaDTO.getDataEntradaFinal() != null
				? filtroPesquisarVisitanteEntradaDTO.getDataEntradaFinal()
				: null;
		;
		LocalTime horaEntradaInicial = filtroPesquisarVisitanteEntradaDTO.getHoraEntradaInicial() != null
				? filtroPesquisarVisitanteEntradaDTO.getHoraEntradaInicial()
				: null;
		LocalTime horaEntradaFinal = filtroPesquisarVisitanteEntradaDTO.getHoraEntradaFinal() != null
		...
		List<ResultadoPesquisaVisitanteEntradaDTO> entradas = crepo.pesquisarVisitantesEntradas(nomeVisitante, numeroDocumentoVisitante,
		...
		for (ResultadoPesquisaVisitanteEntradaDTO ent : entradas) {
			

			
			ent.setFotoVisitante(fotoVisitanteService.getUltimaFotoVisitante(ent.getId()).getImagemFoto());
			
			DateTimeFormatter dataFormatada = DateTimeFormatter.ofPattern("dd/MM/yyyy");
			DateTimeFormatter horaFormatada = DateTimeFormatter.ofPattern("HH:mm:ss");

			ent.setDataEntrada(ent.getDataVisita().format(dataFormatada));
			ent.setHoraEntrada(ent.getDataVisita().format(horaFormatada));
	...
	private String buscarNomeRestricao(Integer entradaID) {
		
		String nomeRestricao = null;
		
		if(entradaID != null) {
		 nomeRestricao = restricaoRepository.buscarRestricaoPorEntradaID(entradaID);
		}
		return nomeRestricao != null ? nomeRestricao: "-";
	}
	...
	public EntradaDTO buscarEntradaID(Map<String, String[]> parameterMap) throws IOException {
		...
		VisitanteDTO visitanteDTO = modelMapper.map(entrada.get().getVisitante(), VisitanteDTO.class);
		visitanteDTO.setFotoVisitante(fotoVisitanteService.getUltimaFotoVisitante(visitanteDTO.getId()).getImagemFoto());
		retorno.setVisitanteDTO(visitanteDTO);
		
		if(entrada.get().getIdGabinete() != null) {
			List<GabineteDTO> gabinetes = gabineteService.findGabinete();
			Supplier<Stream<GabineteDTO>>  gabineteDTO = () -> gabinetes.stream().filter(gabinete -> gabinete.getIdeCadastro().equals(entrada.get().getIdGabinete()));
			retorno.setGabineteDTO(gabineteDTO.get().findAny().get());
		}
