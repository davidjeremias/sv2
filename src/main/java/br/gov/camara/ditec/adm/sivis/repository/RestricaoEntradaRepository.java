...
public interface RestricaoEntradaRepository extends JpaRepository<RestricaoEntrada, Integer> {
	...
	@Query("SELECT res FROM RestricaoEntrada res " + " LEFT JOIN res.visitante vis " + " JOIN res.tipoRestricao tres "

			+ " WHERE ((res.numCPF = :cpf) OR (vis.cpf = :cpf)) " + " AND (:idTipoRestricao IS NULL OR tres.id = :idTipoRestricao) "
...
//			+ " from RestricaoEntrada re where re.id = res.id )"
	...
	@Query("SELECT  tres.nome FROM TentativaEntradaRestricao tentativa " 
			+ " INNER JOIN tentativa.restricaoEntrada res " 
			+ " LEFT JOIN res.tipoRestricao tres " 
			+ " WHERE tentativa.entrada.id = :entradaID ")
	String buscarRestricaoPorEntradaID(@Param("entradaID") Integer entradaID);
