...
public class EntradaCustomRepositoryImpl implements EntradaCustomRepository{
	...
	@Override
	public List<ResultadoPesquisaVisitanteEntradaDTO> pesquisarVisitantesEntradas(String nomeVisitante, String numeroDocumentoVisitante,
...
//				//+ " INNER JOIN e.visitante visitante"
...
//		
		...
		queryHQL.append(" SELECT DISTINCT NEW br.gov.camara.ditec.adm.sivis.service.dto.ResultadoPesquisaVisitanteEntradaDTO"
				+ "(e.id ,"
				+ "e.visitante.id,"
				+ " e.visitante.nomeVisitante,"
				+ " e.visitante.cpf,"
				+ " tpre.nome,"
				+ " e.dataHoraVisita,"
				+ " e.portaria.descricaoPortaria,"
				+ " destino.nomeDestino ) "
				+ " FROM Entrada e "
				+ " LEFT JOIN TentativaEntradaRestricao tentativa ON tentativa.entrada.id = e.id "
				+ " LEFT JOIN RestricaoEntrada res ON res.id = tentativa.restricaoEntrada.id "
				+ " LEFT JOIN TipoRestricao tpre ON tpre.id = res.tipoRestricao.id"
				+ " LEFT JOIN Documento d ON e.visitante = d.visitante "
				+ " LEFT JOIN Destino destino ON e.destino.id = destino.id "
				+ "	  "
				+ " WHERE e.visitante.dataCadastro is not null "
				);
		...
		if(tipoRestricaoDTO != null) {
			...
			where.append(" AND tpre.id = "+ tipoRestricaoDTO.getId()+ "");}
		}
		...
		if (dataEntradaInicial != null && dataEntradaFinal == null) {
			where.append(" AND (CONVERT(date,e.dataHoraVisita) >= '" + dataEntradaInicial + "')");
		} else if (dataEntradaInicial == null && dataEntradaFinal != null) {
			where.append(" AND (CONVERT(date,e.dataHoraVisita) <= '" + dataEntradaFinal + "')");
		} else if (dataEntradaInicial != null && dataEntradaFinal != null) {
			where.append(
					" AND CONVERT(date,e.dataHoraVisita) BETWEEN '" + dataEntradaInicial + "' AND '" + dataEntradaFinal + "'");
		}

		if (horaEntradaInicial != null && horaEntradaFinal == null) {
			where.append(" AND (CONVERT(time(4),e.dataHoraVisita) >= '" + horaEntradaInicial + "')");
		} else if (horaEntradaInicial == null && horaEntradaFinal != null) {
			where.append(" AND (CONVERT(time(4),e.dataHoraVisita) <= '" + horaEntradaFinal + "')");
		} else if (horaEntradaInicial != null && horaEntradaFinal != null) {
			where.append(
					" AND CONVERT(time(4),e.dataHoraVisita) BETWEEN '" + horaEntradaInicial + "' AND '" + horaEntradaFinal + "'");
		}
		...
		if (destinoDTO.getId() != null) {
			where.append(" AND destino.id =" + destinoDTO.getId() + " ");
		}
		...
		Query query = em.createQuery(queryHQL.toString(), ResultadoPesquisaVisitanteEntradaDTO.class);

		List<ResultadoPesquisaVisitanteEntradaDTO> retorno = query.getResultList();
