...
public interface EntradaRepository extends JpaRepository<Entrada, Integer> {
	...
	@Query(value = "SELECT e FROM Entrada e WHERE e.visitante.id = :idVisitante AND e.agendamento.id = :idAgendamento " +
			"AND e.dataHoraVisita = (SELECT MAX(e.dataHoraVisita) FROM Entrada e WHERE e.visitante.id = :idVisitante AND e.agendamento.id = :idAgendamento) ")
