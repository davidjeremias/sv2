...
public class VisitanteCustomRepositoryImpl implements VisitanteCustomRepository {
	...
	public PageImpl<Visitante> findAllFilter(Pageable pageable, String nome, String numero) {
		...
		queryHQL.append(" SELECT DISTINCT v FROM Visitante v LEFT JOIN Documento d ON v.id = d.visitante WHERE");
		countHQL.append(" SELECT COUNT(v) FROM Visitante v LEFT JOIN Documento d ON v.id = d.visitante WHERE");

		where.append(" v.nomeVisitante ='" + StringUtil.trataAspas(nome.trim()) + "'");
